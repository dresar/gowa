...
const extractPhone = (payload: any): string | null => {
  if (payload?.sender_id) return String(payload.sender_id);
...
const extractText = (payload: any): string | undefined => {
  ...
  if (payload?.message?.conversation) return String(payload.message.conversation);
  if (payload?.message?.extendedTextMessage?.text) return String(payload.message.extendedTextMessage.text);
  return undefined;
};
...
export const handleWebhookEvent = async (body: any) => {
  ...
  if (isMessage) {
    ...
    if (phone && rawText) {
      // Get device ID if available in payload
      const deviceId = body?.device_id || body?.id || body?.payload?.device_id;
      
      const reply = matchReply(rawText, deviceId, phone);
      debugLog("Match reply result:", { reply, deviceId, phone });
      
      if (reply) {
        try {
          // Pass deviceId to sendTextMessage so it uses the same device that received the message
          const result = await sendTextMessage({ phone, message: reply, device_id: deviceId });
